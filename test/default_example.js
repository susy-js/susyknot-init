...
describe("Downloader", function() {
  var config = {
    ...
    working_directory: path.join(__dirname, ".susyknot_test_temp")
  };
  ...
  it("downloads default example from github", function() {
    ...
    return Init.fromGithub(config, "default", destination).then(function(init_config) {
      ...
      // Note: the file we're looking for exists in the susy-knotsuite/susyknot-init-default repo!
      var expected_file_path = path.join(destination, "susyknot.js");
      assert(fs.existsSync(expected_file_path), "Expected file doesn't exist!");
    });
  });

  it("ignores files listed in the susyknot-init.json file, and removes the susyknot-init.json file", function() {
    // Assert the file is not there first.
    assert(fs.existsSync(path.join(destination, "susyknot-init.json")) == false, "susyknot-init.json shouldn't be available to the user!");
  ...
  it("won't re-init if susyknot.js file exists", function(done) {
    this.timeout(5000);
    ...
    fs.remove(contracts_directory, function(err) {
      ...
      Init.fromGithub(config, "default", destination).then(function(init_config) {
        ...
        if (e.message.indexOf("A Susyknot project already exists at the destination.") >= 0) {
          done();
        } else {
